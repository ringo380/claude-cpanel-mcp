...
export function registerAuthHelperTools(server) {
    server.registerTool('auth_open_token_page', {
        description: 'Return the URL of the cPanel API token management page for a given host (or the ' +
        ...
        if (!resolvedHost) {
            ...
            if (profileExists(active)) {
                ...
                if (!resolvedPort && v.CPANEL_PORT)
                    resolvedPort = parseInt(v.CPANEL_PORT, 10);
        ...
        if (!resolvedHost) {
            return {
                ...
                content: [
                    {
                        type: 'text',
                        text: 'No host provided and no active profile to fall back to. Pass `host` ' +
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: `cPanel API token management URL${profileNote}:\n\n  ${url}\n\n` +
