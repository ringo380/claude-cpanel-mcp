    ...
    'mysql_create_user',
    {
      ...
      inputSchema: {
        name: z
          .string()
          .describe(
            'Bare user name, without the cPanel prefix - cPanel prepends "<cpanel_user>_" itself. ' +
              'Every other MySQL tool wants the full prefixed name as shown by mysql_list_users.',
          ),
        password: z
          .string()
          .describe('Password for the new user. Routed via POST, so it never reaches the cPanel access log.'),
      },
    },
    async ({ name, password }) => {
      const client = getClient();
    ...
    async ({ name, confirm }) => {
      ...
      if (!confirm) {
        return {
          isError: true,
          content: [
            { type: 'text' as const, text: 'Refusing to delete: pass `confirm: true` to acknowledge irreversible data loss.' },
          ],
        };
      }
      try {
  ...
  server.registerTool(
    ...
    async ({ name, confirm }) => {
      ...
      if (!confirm) {
        return {
          isError: true,
          content: [{ type: 'text' as const, text: 'Refusing to delete: pass `confirm: true`.' }],
        };
      }
      try {
    ...
    async ({ user, database }) => {
      ...
      try {
        return asJsonContent(
          await client.call('Mysql', 'revoke_access_to_database', { user, database }),
        );
      } catch (err) {
        return asErrorContent(err);
      }
