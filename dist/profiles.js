...
export function assertValidProfileName(name) {
    if (!VALID_NAME.test(name)) {
        throw new Error(`Invalid profile name "${name}". Use 1-64 chars of [a-zA-Z0-9_.-] only.`);
    }
}
...
function migrateLegacyIfNeeded() {
    ...
    if (!legacy.startsWith('# DEPRECATED:')) {
        const header = '# DEPRECATED: cpanel-mcp now reads profiles/default.env. ' +
...
export function getActiveProfileName() {
    ...
    if (envOverride && VALID_NAME.test(envOverride))
        return envOverride;
    try {
        const af = activeFile();
        if (fs.existsSync(af)) {
 ...
 */
export function writeProfileAtomic(name, values) {
    assertValidProfileName(name);
    const dir = profilesDir();
    fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
    try {
...
export function deleteProfile(name) {
    ...
    if (name === getActiveProfileName()) {
        throw new Error(`Refusing to delete the active profile "${name}". Switch to a different profile first.`);
    }
    const p = profilePath(name);
    if (fs.existsSync(p)) {
        fs.unlinkSync(p);
