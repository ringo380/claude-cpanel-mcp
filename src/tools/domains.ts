...
export function registerDomainTools(server: McpServer, getClient: GetClient): void {
  server.registerTool(
    'domains_list_all',
    {
    ...
    async ({ domain, rootdomain, dir }) => {
      ...
      try {
        return asJsonContent(
          await client.call('SubDomain', 'add_subdomain', { domain, rootdomain, dir }),
        );
      } catch (err) {
        return asErrorContent(err);
      }
  ...
  server.registerTool(
    ...
    async ({ newdomain, subdomain, dir, pass }) => {
      ...
      try {
        return asJsonContent(
          await client.call('AddonDomain', 'add_addon_domain', { newdomain, subdomain, dir, pass }),
        );
      } catch (err) {
        return asErrorContent(err);
      }
