...
export function registerFtpTools(server: McpServer, getClient: GetClient): void {
  server.registerTool(
    'ftp_list',
    {
    ...
    async ({ user, pass, homedir, quota }) => {
      ...
      try {
        return asJsonContent(
          await client.call('Ftp', 'add_ftp', { user, pass, homedir, quota: quota ?? 0 }),
        );
      } catch (err) {
        return asErrorContent(err);
      }
  ...
  server.registerTool(
    ...
    async ({ user, destroy }) => {
      ...
      try {
        return asJsonContent(
          await client.call('Ftp', 'delete_ftp', { user, destroy: destroy ? 1 : 0 }),
        );
      } catch (err) {
        return asErrorContent(err);
      }
