...
import { CONFIG_FILE, readConfig, tokenManagementUrl, validateConfigEphemeral, writeConfigFile, } from '../config.js';
import { assertValidProfileName, deleteProfile, getActiveProfileName, profileExists, readProfile, setActiveProfile, summarizeProfiles, } from '../profiles.js';
...
export function registerSetupTools(server, setClient, onConfigured) {
    server.registerTool('setup', {
        description: 'Interactive setup: validate and save cPanel credentials (host, username, API token). ' +
    ...
    server.registerTool('auth_status', {
        description: 'Report whether cpanel-mcp is configured and where credentials are loaded from ' +
    ...
    server.registerTool('auth_test', {
        description: 'Dry-run credential validation against cPanel UAPI without writing anything to disk. ' +
        ...
        inputSchema: {
            ...
            insecure_tls: z
                .boolean()
                .optional()
                .describe('Skip TLS certificate verification. Only for a self-signed host you control - it leaves the API token exposed to anyone able to intercept the connection.'),
        },
    ...
    server.registerTool('auth_rotate_token', {
        description: 'Atomically swap the API token for an existing profile. Re-uses the profile\'s ' +
        ...
        const config = { host, port, user, apiKey: api_key, insecureTls };
        const result = await validateConfigEphemeral(config);
        if (!result.ok) {
            ...
            return {
                ...
                content: [
                    {
                        type: 'text',
                        text: `Rotation REJECTED: new token failed validation [${result.code}] ${result.message}\n\n` +
    ...
    server.registerTool('auth_list_profiles', {
        description: 'List all saved cPanel credential profiles with host/user/last-4-token, and which one is active.',
        inputSchema: {},
    }, async () => {
        const profiles = summarizeProfiles();
    ...
    server.registerTool('auth_switch_profile', {
        description: 'Switch the active cPanel profile. Reinstantiates the in-memory client immediately ' +
    ...
    server.registerTool('auth_delete_profile', {
        ...
        inputSchema: {
            profile: z
                .string()
                .describe('Name of the profile to delete, as listed by auth_list_profiles. Cannot be the active profile - switch with auth_switch_profile first.'),
            confirm: z
                .boolean()
