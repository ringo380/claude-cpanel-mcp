...
export function registerSetupTools(
        ...
        content: [
          {
            type: 'text' as const,
            text:
              `Success — credentials validated and saved to ${path} (mode 0600).\n\n` +
  ...
  server.registerTool(
    ...
    {
      description:
        'Report whether cpanel-mcp is configured and where credentials are loaded from (process env vs ~/.config/cpanel-mcp/.env). No network call. Use to diagnose unconfigured state or to confirm credential source before changing it.',
      inputSchema: {},
    },
    async () => {
