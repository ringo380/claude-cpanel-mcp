...
function unconfiguredResult() {
    return {
        ...
        content: [
            {
                type: 'text',
                text: 'cpanel-mcp is not configured. Set CPANEL_HOST, CPANEL_USER, and CPANEL_API_KEY ' +
                    'in your environment (or run /cpanel-mcp:setup), then reconnect the MCP server.',
            },
        ],
    };
}
function asJsonContent(data) {
    return {
        content: [{ type: 'text', text: JSON.stringify(data, null, 2) }],
    };
}
function asErrorContent(err) {
...
export function registerGenericTools(server, getClient) {
    ...
    server.registerTool('uapi_call', {
        description: 'Universal cPanel UAPI escape hatch. Calls https://<host>:2083/execute/<module>/<function> with the given params and returns parsed JSON. Use this when no curated tool covers your need. Will throw on cPHulk lockout (CPHULK_LOCKOUT) or auth failure (AUTH_FAILED).',
        inputSchema: {
